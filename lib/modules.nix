...
in rec {
  mapModules = dir: fn:
    mapFilterAttrs (n: v: v != null && !(hasPrefix "_" n)) (n: v:
      ...
      else if v == "regular" && !(hasPrefix "_" n) && n != "default.nix"
  ...
  mapModulesRec' = dir: fn: ignore:
    let
      dirs = mapAttrsToList (k: _: "${dir}/${k}") (filterAttrs
        (n: v: v == "directory" && !(hasPrefix "_" n) && !(ignore n v))
        (readDir dir));
      files = attrValues (mapModules dir id);
      paths = files ++ concatLists (map (d: mapModulesRec' d id ignore) dirs);
    in map fn paths;

  ignoreDarwin = n: v: n == "darwin";
  ignoreLinux = n: v: n == "linux";
}
